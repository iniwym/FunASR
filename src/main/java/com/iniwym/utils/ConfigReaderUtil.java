...
public class ConfigReaderUtil {
    ...
    private static void loadPropertyConfig() {
        try (InputStream input = ConfigReaderUtil.class.getClassLoader().getResourceAsStream("paramMapping.properties")) {
            if (input == null) {
    ...
    private static void loadJsonConfig(String env) {
        try {
            if (ENV_SOURCE.equalsIgnoreCase(env)) {
                ...
                // 从文件中读取默认配置
                try (InputStream input = ConfigReaderUtil.class.getClassLoader().getResourceAsStream("user_info.json")) {
                    if (input == null) {
